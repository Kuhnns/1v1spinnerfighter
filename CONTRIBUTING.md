...
2. Run `node tests/verify.mjs`, `node tests/verify-stats.mjs`, `node tests/verify-roster-combat.mjs`, `node tests/verify-online.mjs`, and `node tests/verify-static.mjs`.
...
6. Exercise the damage matrix boundaries, tied-stat Power Clash, four- and six-tier Speed Blitz thresholds, Boundless immunity, Boundless Double KO, Last Stand presentation, and a full winner-stays-on battle whenever combat logic changes.
